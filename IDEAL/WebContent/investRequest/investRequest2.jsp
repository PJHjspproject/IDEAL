...
// 	var storyguide = document.getElementById("storyguide");//<aside>태그 부분의 도움말이 들어갈 부분
	var $storyguide = $("#storyguide");
	if(namevalue=="addmainbtn"&&nchk==1){//여기서 nchk는 상태 체크용 변수. 만약 상태체크가 되지 않는다면 클릭할 때 마다 도움말 부분에 내용이 계속 추가된다.
		d.style.backgroundImage = "url('none')";
		if(nchk==1)nchk+=1;//상태 체크 및 상태 체크용 변수값 변경
// 		$storyguide.css({"display":"block", "overflow":"hidden", "height":"0px"});
		//min-height, padding:20px;
		$("#addmainbtn").removeClass("animated infinite flash")
		$("#mainImageAdd").css("display","block");
		$storyguide.append("<h3>1. 캠페인 소개</h3><br><p>- 캠페인 전체를 요약하는 간략한 소개를 해주세요.</p><br>"
													+"<p>- 이 캠페인가 여러분에게 왜 중요한가요?</p><br>"
							+"<h3>2. 캠페인 상세 소개</h3><br><p>- 이 캠페인를 통해 무엇을 하고자 하나요?</p><br>"
							...
							+"<h3>3. 팀소개</h3><br><p>- 캠페인를 함께 진행할 구성원들을 소개해주세요.</p><br>"
												+"<p>- 팀이 아닌 혼자일 경우에도 경력이나 프로필 등 당신이 누군지 소개해주세요.</p><br>");
		$storyguide.animate({height:"toggle",
							overflow:"visible",
							padding:"20px",
							display:"block"},
							500);
...
// 				alert(f.elements[i].style.display);
				...
				var msg = filecnt+"번째 대표 썸네일 이미지 파일 정보가 누락되었습니다.\n올바른 입력을 해주세요.";//
				alert(msg);
				//포커스 주기
				f.elements[i].focus();
				...
				var msg = (filecnt-5)+"본문 내용 이미지 파일 정보가 누락되었습니다.\n올바른 입력을 해주세요.";
...
function mainImagePreview(d){
	var previewimg = d.previousSibling;//파일 업로드시 미리보기를 뿌려줄 <input type="file">태그의 이전 형제 <img>태그
// 	alert(d.value);
...
$(function(){
	var mainImageCnt = 0;
	$("#mainImageAdd").on("click", function(){
		$("#div_mainImageAdd").append("<img src=''><input type='file' id='mainImage' name='mainImage"+mainImageCnt+"' onchange='mainImagePreview(this)'><Br>");
		mainImageCnt+=1;
// 		alert(mainImageCnt);
	})
	
	$("#addmainbtn").addClass("animated infinite flash");//본문 상세 내용 넣는 부분에 클릭을 유도하게끔 animate.css적용(infinite + flash)
})
</script>
</head>
<body>
		...
		<section>
			<div class="section_wrap">
				...
				<form action="investRequestAction2.iR" method="post" enctype="multipart/form-data">
					...
					<div>
						<!-- 대표 이미지 썸네일 출력 부분 -->
						<div id="holder" class="thumb_none"><br><br><br><br>720*380 사이즈 / 2M미만 / 영문,숫자 파일명 권장 / 등록가능 확장자 png, gif, jpg</div>
						<!-- 대표 이미지 입력받는 부분 -->
						<div id="div_add">
							<div id="mainThumbdiv">
...
<!-- 								</div> -->
					...
					<!-- 스토리(본문내용) 입력받는 부분 -->
					<div id="div_mainadd">
						<input type="button" id="addmainbtn" value="캠페인 상세 소개" name="addmainbtn" onclick="openAddMain(this)">
						<div id="div_mainImageAdd">
<!-- 							<img src=""><br> -->
<!-- 							<input type="file" id="mainImage" name="mainImage" onchange="mainImagePreview(this)"><br> -->
						</div>
						<input type="button" id="mainImageAdd" value="사진 추가하기">
					</div>
					<br>
					<div id="submitdiv">
		...
		<aside class="junside">
			<div id="asidewrap">
				...
				<br><br>
				<p>
					570*380 사이즈 / 2M미만 / 영문,숫자 파일명 권장 / 등록가능 확장자 png, gif, jpg
				</p>
				<br><br>
